...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $idColegio = $_POST['idColegio'] ?? null; // ID del colegio a actualizar

    if (empty($idColegio)) {
        $message = "Error: ID de colegio no recibido, no se pudo actualizar.";
        $message_type = 'danger';
    } else {
        try {
            switch ($_POST['form_action'] ?? '') {
                case 'update_exam':
                    ...
                    $stmt = $conn->prepare("UPDATE colegio SET anioautoweb = ?, iDturnoautoweb = ?, inscExamDesde = ?, inscExamLectDesde = ?, inscExamHasta = ? WHERE idColegio = ?");
                    $stmt->bind_param("iisssi", $anioautoweb, $iDturnoautoweb, $inscExamDesde, $inscExamLectDesde, $inscExamHasta, $idColegio);
                ...
                case 'update_cursado':
                    $anioautoweb = $_POST['anioautoweb_cursado'] ?? null; // Mismo campo, pero podríamos tenerlo en un select diferente
                    ...
                    $stmt = $conn->prepare("UPDATE colegio SET anioautoweb = ?, inscCursDesde = ?, inscCursLectDesde = ?, inscCursHasta = ? WHERE idColegio = ?");
                    $stmt->bind_param("isssi", $anioautoweb, $inscCursDesde, $inscCursLectDesde, $inscCursHasta, $idColegio);
                ...
                case 'update_actas':
                    ...
                    $stmt = $conn->prepare("UPDATE colegio SET anio_carga_notas = ?, cargaActaVolTurno = ?, cargaActaVolDesde = ?, cargaActaVolHasta = ? WHERE idColegio = ?");
                    $stmt->bind_param("iisssi", $anio_carga_notas, $cargaActaVolTurno, $cargaActaVolDesde, $cargaActaVolHasta, $idColegio);
...
if ($result && $result->num_rows > 0) {
    ...
    // Si no se encuentra el colegio, inicializar con valores vacíos para evitar errores
...
<head>
  ...
  <link rel="stylesheet" href="../css/estilos.css"> <!-- Tu archivo de estilos unificado -->
      ...
      <div class="card shadow-sm h-100">
        ...
        <div class="card-body">
          <form method="POST" action="">
            <input type="hidden" name="idColegio" value="<?php echo htmlspecialchars($colegioId); ?>">
